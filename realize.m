...
invoke(solver, 'FrequencyRange', "2", "18");
release(solver);
...
% 获取最后一行第一列的值
realstructure = data(end, 2:4);
% [SP] = fun_invoke(mws, 7, 6.40, 0.363, count);
[SP] = fun_invoke(mws, realstructure(1), realstructure(2), realstructure(3), count);
x = SP(:,1)';
y = SP(:,2)';
for i = 1:1:length(x)
        [S21(i),S11(i)] = LCmedisingle(C1,C2,L1,L2,x(i),a1,a2,er1,er2,ff0,theta,theta0);
end
figure
h1 = plot(x,S21,'r','linewidth',2);
hold on;
h2 = plot(x,y,'b','linewidth',2);
grid on;
legend([h1 h2], '传输线结果', 'CST结果')
